...
public class CoreBots
{
    ...
    private Task? _fastLoopTask;
    private Task? _slowLoopTask;
    private readonly object _syncLock = new();
    private string? _charLocDir;
    //butler-sync end

    // Cached server name for Relogin (captured once at startup after test-server validation)
    ...
    private void InitializeButlerSync()
    {
        ...
        {
            _charLocDir = Path.Combine(
                Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),
                "Skua",
                "character_locations"
            );
            Directory.CreateDirectory(_charLocDir);
            ...
            string GetSyncFilePath()
            {
                ...
                return Path.Combine(_charLocDir, $"{username}_location.sync");
            }
            ...
            bool TryGetRoomNumber(out string roomNumber)
            {
                ...
                {
                    Bot.Log($"[ButlerSync] TryGetRoomNumber: {ex.Message}");
                    return false;
                }
            }
            ...
            void WriteLocation(bool forceTimestampRefresh = false)
            {
                try
                {
                    ...
                    if (locationChanged || forceTimestampRefresh)
                    {
                        ...
                        string path = GetSyncFilePath();
                        string tempPath = path + ".tmp";

                        lock (_syncLock)
                        {
                            using (FileStream stream = new(tempPath, FileMode.Create, FileAccess.Write, FileShare.Read))
                            using (StreamWriter writer = new(stream))
                                writer.Write(currentLocation);

                            const int maxFileRetries = 5;
                            const int fileRetryDelayMs = 200;
                            bool fileSwapDone = false;

                            for (int retry = 0; retry < maxFileRetries; retry++)
                            {
                                try
                                {
                                    if (File.Exists(path))
                                ...
                                catch (IOException) when (retry < maxFileRetries - 1)
                                {
                            ...
                            if (!fileSwapDone)
                                Bot.Log($"[ButlerSync] WriteLocation: failed to replace sync file after {maxFileRetries} attempts");
                        }
                ...
                {
                    Bot.Log($"[ButlerSync] WriteLocation: {ex.Message}");
                }
            }
            ...
            // FAST LOOP — polls for state changes every 500ms
            _fastLoopTask = Task.Run(async () =>
            {
                ...
                while (!token.IsCancellationRequested)
                {
                    ...
                    {
                        Bot.Log($"[ButlerSync] FastLoop: {ex.Message}");
                    }
                }
            }, _locationSyncCTS.Token);

            // SLOW LOOP — heartbeat, refreshes timestamp every 5s
            _slowLoopTask = Task.Run(async () =>
            {
                var token = _locationSyncCTS.Token;
                while (!token.IsCancellationRequested)
                {
                    ...
                    {
                        Bot.Log($"[ButlerSync] SlowLoop: {ex.Message}");
                    }
                }
            }, _locationSyncCTS.Token);
        }
    ...
    /// <param name="disableCoreSkills"></param>
    public void SetOptions(bool changeTo = true, bool disableClassSwap = false, bool disableCoreSkills = false)
    {
        ...
        if (changeTo)
        {
            ...
            const int waitDelayMs = 10 * 60 * 1000;

            string server = Bot.Servers.LastName;
            int loginAttempt = 0;
            ...
            while (!Bot.Player.LoggedIn && !Bot.ShouldExit)
            {
                ...
                {
                    int slept = 0;
                    while (!Bot.ShouldExit && slept < retryDelayMs)
                    {
                        int step = Math.Min(ActionDelay, retryDelayMs - slept);
                        Sleep(step);
                        slept += step;
                    }
                    continue;
                ...
                Bot.Log($"⏰ Login failed {maxLoginAttempts} times. Waiting 10 minutes before retrying.");
                int totalSlept = 0;
                while (!Bot.ShouldExit && totalSlept < waitDelayMs)
                {
                    int step = Math.Min(ActionDelay, waitDelayMs - totalSlept);
                    Sleep(step);
                    totalSlept += step;
                }
       ...
       */
        ...
        // Cache the server name after confirming we're on a valid (non-test) server
        _cachedServerName = Bot.Servers.LastName;
    ...
    private void StopButlerSync()
    {
        // Signal background loops to stop
        if (_locationSyncCTS != null)
        {
            _locationSyncCTS.Cancel();
            _locationSyncCTS.Dispose();
            _locationSyncCTS = null;
        }

        // Reset static flag so a future script can restart sync
        _syncWriterStarted = 0;

        // Clean up task references (cancellation propagates, no blocking wait needed)
        _fastLoopTask = null;
        _slowLoopTask = null;

        // Delete the sync file for this account
        try
        {
            string username = Bot.Player?.Username;
            if (string.IsNullOrWhiteSpace(username))
            ...
            if (!string.IsNullOrWhiteSpace(username) && _charLocDir != null && Directory.Exists(_charLocDir))
            {
                string syncFile = Path.Combine(_charLocDir, $"{username}_location.sync");
        ...
        {
            Bot.Log($"[ButlerSync] Cleanup: {ex.Message}");
        }
    }
...
#nullable enable
    ...
    */
    ...
    private static readonly TimeSpan PreferredReloginDelay = TimeSpan.FromMinutes(10);
    ...
    public void Relogin(string reason = "")
    {
        ...
        try
        {
            ...
            string preferredServer = _cachedServerName ?? Bot.Servers.LastName;
            Bot.Log($"🎯 Persistent relogin target: {preferredServer} 🔁");

            const int reloginAttemptsBeforeDelay = 3;
            int reloginAttempt = 0;

            while (!Bot.ShouldExit)
            {
                reloginAttempt++;
                Bot.Log($"🔄 Attempt {reloginAttempt}/{reloginAttemptsBeforeDelay} to relogin to: {preferredServer}");
                bool relogSuccess = false;
                try
                {
                    using var cts = new CancellationTokenSource();
                    relogSuccess = Bot.Wait.ForTrue(() => Bot.Servers.EnsureRelogin(cts.Token).Result, 20);
                }
                catch (Exception ex)
                {
                    Bot.Log($"❌ Relogin attempt threw: {ex.Message}");
                }

                if (relogSuccess && Bot.Wait.ForTrue(() => Bot.Player?.Loaded ?? false, 20))
                {
                ...
                if (reloginAttempt < reloginAttemptsBeforeDelay)
                {
                    Bot.Log($"⏳ Relogin attempt {reloginAttempt} failed. Retrying in 5 seconds.");
                ...
                Bot.Log(
                    $"⏰ Relogin to {preferredServer} failed {reloginAttemptsBeforeDelay} times. Waiting {PreferredReloginDelay.TotalMinutes:0} minutes before next round."
                );

                int waitTimeMs = (int)PreferredReloginDelay.TotalMilliseconds;
                int totalSlept = 0;
                while (!Bot.ShouldExit && totalSlept < waitTimeMs)
                {
                    int step = Math.Min(ActionDelay, waitTimeMs - totalSlept);
                    Sleep(step);
                    totalSlept += step;
                }
            ...
            Bot.Log("🛑 Relogin aborted because the script is stopping.");
        }
        finally
        {
...
public class CoreSkills
{
    ...
    private void ArchmageClass()
    {
        if (
            ...
            && IsHealthHigh(80)
            && (!HasAura("Arcane Sigil", true) || !HasAura("Corporeal Ascension", true))
        )
            if (Cast(4))
                return;
        ...
        if (!HasAura("Arcane Sigil", true) && IsHealthHigh(80))
            if (Cast(4))
                return;
